  ...
  fiducial_vlam_msgs__msg__Observation__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    fiducial_vlam_msgs__msg__Observation * data =
      (fiducial_vlam_msgs__msg__Observation *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!fiducial_vlam_msgs__msg__Observation__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          fiducial_vlam_msgs__msg__Observation__fini(&output->data[i]);
        }
        return false;
      }
    }
