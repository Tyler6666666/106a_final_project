  ...
  void * untyped_ros_message)
{
  ...
  // Field name: fixed_flags
  {
    ...
    if (!rosidl_runtime_c__int32__Sequence__init(&ros_message->fixed_flags, size)) {
      fprintf(stderr, "failed to create array for field 'fixed_flags'");
      return false;
    }
    auto array_ptr = ros_message->fixed_flags.data;
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = fiducial_vlam_msgs__msg__Map;
    is_plain =
      (
...
static size_t _Map__max_serialized_size(char & bounds_info)
{
  ...
  ret_val = max_serialized_size_fiducial_vlam_msgs__msg__Map(
