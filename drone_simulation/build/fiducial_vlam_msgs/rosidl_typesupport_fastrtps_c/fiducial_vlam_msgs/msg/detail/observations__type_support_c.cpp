  ...
  void * untyped_ros_message)
{
  ...
  // Field name: observations
  {
    ...
    if (!fiducial_vlam_msgs__msg__Observation__Sequence__init(&ros_message->observations, size)) {
      fprintf(stderr, "failed to create array for field 'observations'");
      return false;
    }
    auto array_ptr = ros_message->observations.data;
    for (size_t i = 0; i < size; ++i) {
  ...
  size_t current_alignment)
{
  ...
  if (is_plain) {
    ...
    using DataType = fiducial_vlam_msgs__msg__Observations;
    is_plain =
      (
      offsetof(DataType, observations) +
      last_member_size
      ) == ret_val;
  }
...
static size_t _Observations__max_serialized_size(char & bounds_info)
{
  bool full_bounded;
  bool is_plain;
  size_t ret_val;

  ret_val = max_serialized_size_fiducial_vlam_msgs__msg__Observations(
