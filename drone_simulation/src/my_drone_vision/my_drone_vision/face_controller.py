...
class FaceController(Node):
    ...
    def control_loop(self):
        ...
        err_x = self.apply_deadband(self.latest_pose.position.x, self.deadband_x)
        err_y = self.apply_deadband(self.latest_pose.position.y, self.deadband_y)
        face_area = self.latest_pose.position.z
        ...
        cmd.linear.x = self.make_forward_speed(face_area, err_area)
        self.cmd_pub.publish(cmd)

        if now - self.last_log_time >= 1.0:
            self.get_logger().info(
                f"Face tracking: x={self.latest_pose.position.x:.2f} "
                f"y={self.latest_pose.position.y:.2f} area={face_area:.3f} "
                f"cmd=({cmd.linear.x:.2f}, {cmd.linear.z:.2f}, {cmd.angular.z:.2f})"
            )
            self.last_log_time = now

    def make_forward_speed(self, face_area, err_area):
        ...
        return self.clip(self.kp_forward * err_area, self.max_forward_speed)
